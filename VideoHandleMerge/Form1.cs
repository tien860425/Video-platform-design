    ...
    public partial class Form1 : Form
    {
        ...
        private void GetMessage()
        {
            ...
            if (MessageQueue.Exists(".\\Private$\\MqHandleMerge"))
                myQueue = new MessageQueue(".\\Private$\\MqHandleMerge");
            else
                myQueue = MessageQueue.Create(".\\Private$\\MqHandleMerge");
            ...
            while (!bClose)
            {
                try
                {
                    ...
                    while (true)
                    {
                        if (File.Exists(parse[0] + parse[2] + "_Seg000.mp4"))
                        {
                            log.Info(parse[0] + parse[2] + "_Seg000.mp4 已產生");
                             break;
                       }
                        log.Info(parse[0] + parse[2] + "_Seg000.mp4 未產生");

                        Thread.Sleep(500);
                    }
                    int count = 0;
                    while (true)//1min long video need 16sec
                    {
                        if (File.Exists(parse[0] + parse[2] + "_Seg001.mp4") || count >160* segmentTime/30)
                        {
                            log.Info(parse[0] + parse[2] + "_Seg001.mp4 已產生");
                            break;
                        }
                        log.Info(parse[0] + parse[2] + "_Seg001.mp4 未產生");
                        count++;
                        Thread.Sleep(500);
                    }
                    ...
                    log.Info("開始結合");
                    Thread.Sleep(500);
                    getVideoMerge(parse[0], parse[1], parse[2]);
...
//p.s.留意，這方法不能套用-an或者-vn參數，因為在filter裏面就指定了一條video跟一條audio。
        private void getVideoMerge(string path, string filename, string movieid)
        {
            ...
            try
            {
                ...
                //ffmpeg.exe - i 001.mp4 - i 002.mp4 - filter_complex "[0:v]scale=1280:720,setsar=1/1[v0];[1:v]scale=1280:720,setsar=1/1[v1]; [v0][0:a] [v1][1:a] concat=n=2:v=1:a=1 [v] [a]" - map "[v]" - map "[a]" output.mp4
                p.StartInfo.Arguments = " -i " + path + movieid + "_seg000.mp4 " + "-i \"" + path + filename + "\" -filter_complex \"[0:v]scale=1280:720,setsar=1/1[v0];[1:v]scale=1280:720,setsar=1/1[v1]; [v0][0:a] [v1][1:a] concat=n=2:v=1:a=1 [v] [a]\" -map \"[v]\" -map \"[a]\" " + path + movieid + "_MergeAD.mp4";    //設定程式執行參數
