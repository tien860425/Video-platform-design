        ...
        else if (Request.HttpMethod.ToUpper() == "POST" && Request.QueryString["file"] == null) // ---- POST 的處理片段  -----------------------------------
        {
            try
            {
                ...
                for (int FileIndex = 0; FileIndex < Request.Files.Count; FileIndex++)  //利用 HttpPostedFile 方式來將檔案寫入到 Server
                {
                    ...
                    //for (int i = 0; i < 15; i++)
                    //{
                    //    if (Request.Form["count" +i.ToString()] != null)
                    //    {
                    //        count = Request.Form["count" + i.ToString()];
                    //    }
                    //}
                    string ErrorMessage = String.Empty;
                    string str = hFile.FileName.Replace(" ",string.Empty);
                    string sType = Request.Form["Vtype" + str];
                    string intro = Request.Form["intro" + str];
                    string actor = Request.Form["actor" + str];
                    string title = Request.Form["title" + str];
                    string scut = Request.Form["vcut" + str];
                    string []extL = hFile.FileName.Split('.');
                    string ext = extL[extL.Length - 1].ToUpper();
                    ...
                    string FileName = String.Format(@"{0}{1}", Upload_Directory, MakeFilenameValid(Path.GetFileName(hFile.FileName)));
                    ...
                    lstUpload.Add(Upload_Directory + @";" + MakeFilenameValid(Path.GetFileName(hFile.FileName)) + @";" + movieid + @";" + title + @";" +scut);
                    conn_setup();
